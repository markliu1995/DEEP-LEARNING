...
class MySubTensors:
    def __init__(self, cfg: MyConfig):
        ...
        with tf.variable_scope('disc') as scope:
            x2_y = self.disc(x2, lx)  # 假样本为真的概率, [-1, 1]
            x = tf.reshape(x, [-1, 28, 28, 1])
            scope.reuse_variables()
            x_y = self.disc(x, lx)    # 真样本为真的概率, [-1, 1]

        loss1 = -tf.reduce_mean(tf.log(x_y))
        loss2 = -tf.reduce_mean(tf.log(1 - x2_y))
        loss3 = -tf.reduce_mean(tf.log(x2_y))
