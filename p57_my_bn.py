...
def my_bn(x, axis, training=True, momentum=0.99, epslon=1e-6, name=None):
    ...
    with tf.variable_scope(name):
        ...
        mean = tf.reduce_mean(x, compute_axis)   # [224, 224]
        msd = tf.reduce_mean(tf.square(x), compute_axis)

        shape = [x.shape[e].value if e in axis else 1 for e in range(4)] # [1, 224, 224, 1]
        mean = tf.reshape(mean, shape)   # [1, 224, 224, 1]
        msd = tf.reshape(msd, shape)   # [1, 224, 224, 1]
